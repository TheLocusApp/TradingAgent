...
class VolatilityDivergence(Strategy):
    ...
    def init(self):
        ...
        bb_upper, bb_middle, bb_lower = talib.BBANDS(close_data, timeperiod=self.bb_period, nbdevup=self.bb_std, nbdevdn=self.bb_std, matype=0)
        self.bb_upper = self.I(lambda: bb_upper)
        self.bb_middle = self.I(lambda: bb_middle)
        self.bb_lower = self.I(lambda: bb_lower)
        
        # Calculate BB width
        bb_width_calc = (bb_upper - bb_lower) / bb_middle
        self.bb_width = self.I(lambda: bb_width_calc)
        self.bb_width_avg = self.I(talib.SMA, bb_width_calc.astype(float), timeperiod=20)
        
        self.rsi = self.I(talib.RSI, close_data, timeperiod=self.rsi_period)
        
        macd, macd_signal, macd_hist = talib.MACD(close_data, fastperiod=self.macd_fast, slowperiod=self.macd_slow, signalperiod=self.macd_signal)
...
print("🌙 Moon Dev: Fetching SPY data from yfinance... ✨")
