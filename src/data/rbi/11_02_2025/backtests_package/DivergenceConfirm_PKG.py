...
class DivergenceConfirm(Strategy):
    ...
    def _update_swing_points(self, current_bar):
        ...
        if len(self.price_highs) < 2:
            ...
            self.rsi_highs.append((current_bar, self.rsi[-1]))
            self.rsi_lows.append((current_bar, self.rsi[-1]))
        else:
            ...
            if self.swing_high[-1] == self.data.High[-1]:
                self.price_highs.append((current_bar, self.data.High[-1]))
                self.rsi_highs.append((current_bar, self.rsi[-1]))
                
            if self.swing_low[-1] == self.data.Low[-1]:
                self.price_lows.append((current_bar, self.data.Low[-1]))
                self.rsi_lows.append((current_bar, self.rsi[-1]))
    ...
    def _macd_bullish_confirmation(self):
        ...
        macd_cross = (current_macd > current_macd_signal) and (self.macd[-2] <= self.macd_signal_line[-2] if len(self.macd) > 1 else False)
...
def load_data():
    print("🌙 Fetching BTC-USD data from yfinance...")
    
    # Fetch BTC-USD data for the last 60 days with 15m intervals
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        print("📊 Data Source: yfinance (BTC-USD 15m)")
