...
class DivergenceConfirmation(Strategy):
    ...
    def init(self):
        ...
        # MACD calculation with proper tuple unpacking
        def calc_macd(close):
        ...
        macd_result = self.I(calc_macd, self.data.Close)
    ...
    def _bullish_divergence(self, current_idx):
        ...
        # RSI made higher low
        current_rsi_low = min([x for x in self.rsi[-5:] if x is not None])
        previous_rsi_low = min([x for x in self.rsi[self.last_swing_low[0]-2:self.last_swing_low[0]+2] if x is not None])
    ...
    def _bearish_divergence(self, current_idx):
        ...
        # RSI made lower high
        current_rsi_high = max([x for x in self.rsi[-5:] if x is not None])
        previous_rsi_high = max([x for x in self.rsi[self.last_swing_high[0]-2:self.last_swing_high[0]+2] if x is not None])
    ...
    def _macd_bullish_confirmation(self, current_idx):
        ...
        macd_hist_positive = (self.macd_hist[-1] is not None and self.macd_hist[-1] > 0 and 
                             self.macd_hist[-2] is not None and self.macd_hist[-2] > 0)
        macd_rising = (self.macd_hist[-1] is not None and self.macd_hist[-2] is not None and 
                      self.macd_hist[-1] > self.macd_hist[-2])
        macd_above_signal = (self.macd[-1] is not None and self.macd_signal_line[-1] is not None and 
                            self.macd[-1] > self.macd_signal_line[-1])
    ...
    def _macd_bearish_confirmation(self, current_idx):
        ...
        macd_hist_negative = (self.macd_hist[-1] is not None and self.macd_hist[-1] < 0 and 
                             self.macd_hist[-2] is not None and self.macd_hist[-2] < 0)
        macd_falling = (self.macd_hist[-1] is not None and self.macd_hist[-2] is not None and 
                       self.macd_hist[-1] < self.macd_hist[-2])
        macd_below_signal = (self.macd[-1] is not None and self.macd_signal_line[-1] is not None and 
                            self.macd[-1] < self.macd_signal_line[-1])
        ...
        for trade in self.trades:
            if trade.is_long:
                ...
                macd_exit = (current_idx >= 2 and 
                            self.macd_hist[-1] is not None and self.macd_hist[-1] < 0 and 
                            self.macd_hist[-2] is not None and self.macd_hist[-2] < 0)
...
data = ticker.history(period="6mo", interval="15m")
...
# Ensure proper column mapping and formatting
...
print("🌙 Data prepared for Moon Dev Divergence Strategy! 🚀")
