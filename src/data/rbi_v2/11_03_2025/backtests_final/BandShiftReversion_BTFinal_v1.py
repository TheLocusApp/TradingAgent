...
def load_data():
    ...
    # 🌙 MOON DEV DEBUG: Clean and standardize column names
    data.columns = data.columns.str.strip().str.lower()
    data = data.rename(columns={'date': 'datetime'})
    
    # 🌙 MOON DEV DEBUG: Process datetime properly
    data['datetime'] = pd.to_datetime(data['datetime'])
    data = data.set_index('datetime')
    
    # 🌙 MOON DEV DEBUG: Ensure proper column capitalization for backtesting
    ...
    print(f"🌙 MOON DEV DATA LOADED! Columns: {list(data.columns)}")
...
class BandShiftReversion(Strategy):
    ...
    def init(self):
        ...
        # 🌙 MOON DEV DEBUG: Calculate BBANDS once and extract components
        bb_upper, bb_middle, bb_lower = talib.BBANDS(close, timeperiod=self.bb_period, nbdevup=self.bb_std)
        ...
        if not self.position:
            ...
            if bandshift_condition and touch_condition and bullish_candle:
                ...
                if position_size > 0:
                    ...
                    print(f"💵 Available Equity: {self.equity:.2f}")
                    
                    # 🌙 MOON DEV DEBUG: Execute buy with proper size
