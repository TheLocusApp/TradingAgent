...
class VolatilityDivergence(Strategy):
    ...
    def init(self):
        ...
        # VIX shifted indicator - must return array
        def vix_shift_func(x):
            result = np.empty_like(x)
            for i in range(len(x)):
                if i >= self.vix_lookback:
                    result[i] = x[i - self.vix_lookback]
                else:
                    result[i] = x[0]
            return result
        ...
        # VIX increasing indicator
        def vix_increasing_func(current, shifted):
            return (current > shifted).astype(float)
        
        self.vix_increasing = self.I(vix_increasing_func, vix, self.vix_shifted)
