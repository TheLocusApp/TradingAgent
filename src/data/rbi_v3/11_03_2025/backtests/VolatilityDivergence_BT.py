...
class VolatilityDivergence(Strategy):
    ...
    def init(self):
        ...
        self.divergence_bullish = np.zeros(len(self.data))
        self.divergence_bearish = np.zeros(len(self.data))
    ...
    def next(self):
        ...
        prev_swing_low = self.swing_lows[-2] if len(self.data) > 1 else swing_low
        prev_swing_high = self.swing_highs[-2] if len(self.data) > 1 else swing_high
        
        rsi_low = self.rsi_lows[-1]
        rsi_high = self.rsi_highs[-1]
        prev_rsi_low = self.rsi_lows[-2] if len(self.data) > 1 else rsi_low
        prev_rsi_high = self.rsi_highs[-2] if len(self.data) > 1 else rsi_high
        
        prev_macd_hist = self.macd_hist[-2] if len(self.data) > 1 else current_macd_hist
        ...
        if not self.position:
            ...
            if self.position.is_long:
                exit_condition = (
                    ...
                    current_rsi >= 70 or
                    (self.macd[-1] < self.macd_signal[-1] and self.macd[-2] >= self.macd_signal[-2]) or
