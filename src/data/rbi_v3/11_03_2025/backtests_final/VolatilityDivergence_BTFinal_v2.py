...
class VolatilityDivergence(Strategy):
    ...
    def next(self):
        ...
        prev_swing_low = self.swing_lows[-2] if len(self.swing_lows) > 1 else swing_low
        prev_swing_high = self.swing_highs[-2] if len(self.swing_highs) > 1 else swing_high
        
        rsi_low = self.rsi_lows[-1]
        rsi_high = self.rsi_highs[-1]
        prev_rsi_low = self.rsi_lows[-2] if len(self.rsi_lows) > 1 else rsi_low
        prev_rsi_high = self.rsi_highs[-2] if len(self.rsi_highs) > 1 else rsi_high
