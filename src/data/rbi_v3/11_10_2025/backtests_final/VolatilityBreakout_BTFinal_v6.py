...
class VolatilityBreakout(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            squeeze_cond = bool(self.squeeze_on[-1]) if len(self.squeeze_on) > 0 and not np.isnan(self.squeeze_on[-1]) else False
            bb_upper_val = float(self.bb_upper[-1]) if len(self.bb_upper) > 0 and not np.isnan(self.bb_upper[-1]) else current_close * 1.1
            bb_lower_val = float(self.bb_lower[-1]) if len(self.bb_lower) > 0 and not np.isnan(self.bb_lower[-1]) else current_close * 0.9
            volume_sma_val = float(self.volume_sma[-1]) if len(self.volume_sma) > 0 and not np.isnan(self.volume_sma[-1]) else current_volume
            rsi_val = float(self.rsi[-1]) if len(self.rsi) > 0 and not np.isnan(self.rsi[-1]) else 50.0
            atr_val = float(self.atr[-1]) if len(self.atr) > 0 and not np.isnan(self.atr[-1]) else current_close * 0.01
            ...
            # POSITION MANAGEMENT
            bb_middle_val = float(self.bb_middle[-1]) if len(self.bb_middle) > 0 and not np.isnan(self.bb_middle[-1]) else current_close
            rsi_val = float(self.rsi[-1]) if len(self.rsi) > 0 and not np.isnan(self.rsi[-1]) else 50.0
            atr_val = float(self.atr[-1]) if len(self.atr) > 0 and not np.isnan(self.atr[-1]) else current_close * 0.01
...
try:
    data = load_data()
    if len(data) == 0:
        print("🌙 MOON DEV CRITICAL: No data loaded! Trying alternative ticker...")
        # Try alternative data source or ticker
        import yfinance as yf
        data = yf.download("SPY", period="2y", interval="1h")
        data.columns = data.columns.str.strip().str.lower()
        data = data.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume'})
        data.index = pd.to_datetime(data.index)
    ...
    if len(data) > 0:
        ...
        print("🌙 MOON DEV: No data available for the specified period")
    ...
    if len(data) > 0:
        ...
        print("🌙 MOON DEV: Cannot run backtest - no data available")
        
except Exception as e:
    print(f"🌙 MOON DEV CRITICAL ERROR: {e}")
    print("🌙 MOON DEV: Attempting to load data with alternative method...")
    ...
    try:
        ...
        data = yf.download("SPY", period="2y", interval="1h")
        if len(data) == 0:
            data = yf.download("SPY", period="1y", interval="1h")  # Try shorter period
        data.columns = data.columns.str.strip().str.lower()
        data = data.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume'})
        data.index = pd.to_datetime(data.index)
        
        print(f"🌙 MOON DEV: Alternative load successful - {len(data)} bars")
        if len(data) > 0:
            print(f"🌙 MOON DEV: Data range: {data.index[0]} to {data.index[-1]}")
            bt = Backtest(data, VolatilityBreakout, cash=1000000, commission=.002)
            stats = bt.run()
        ...
        print(f"🌙 MOON DEV: All data loading methods failed: {e2}")