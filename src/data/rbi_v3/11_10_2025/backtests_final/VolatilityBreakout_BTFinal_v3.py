...
class VolatilityBreakout(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            squeeze_cond = self.squeeze_on[-1] if len(self.squeeze_on) > 0 else False
            bb_upper_val = self.bb_upper[-1] if len(self.bb_upper) > 0 else current_close * 1.1
            bb_lower_val = self.bb_lower[-1] if len(self.bb_lower) > 0 else current_close * 0.9
            volume_sma_val = self.volume_sma[-1] if len(self.volume_sma) > 0 else current_volume
            rsi_val = self.rsi[-1] if len(self.rsi) > 0 else 50
            atr_val = self.atr[-1] if len(self.atr) > 0 else current_close * 0.01
            ...
            # POSITION MANAGEMENT
            bb_middle_val = self.bb_middle[-1] if len(self.bb_middle) > 0 else current_close
            rsi_val = self.rsi[-1] if len(self.rsi) > 0 else 50
            atr_val = self.atr[-1] if len(self.atr) > 0 else current_close * 0.01
