    ...
    def next(self):
        ...
        if not self.position:
            if is_squeeze and volume_confirmed:
                ...
                elif current_close < bb_lower and rsi > 30:
                    self.breakout_signal = -1
                ...
                if self.breakout_signal == 1:
                    ...
                    if stop_loss >= entry_price or take_profit <= entry_price:
                        print(f"🌙 MOON DEV: Long order validation failed - SL: {stop_loss:.2f}, Entry: {entry_price:.2f}, TP: {take_profit:.2f}")
                        self.breakout_signal = 0
                        return
                    ...
                    if risk_per_share > 0:
                        ...
                        max_position_size = int(self.equity * 0.1 / entry_price)
                        position_size = min(position_size, max_position_size)
                        
                        if position_size > 0:
                            self.buy(size=position_size, sl=stop_loss, tp=take_profit)
                            print(f"🚀 MOON DEV: ENTERED LONG - Size: {position_size}, Entry: {entry_price:.2f}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}")
                            self.breakout_signal = 0
                    ...
                    if take_profit >= entry_price or stop_loss <= entry_price:
                        print(f"🌙 MOON DEV: Short order validation failed - TP: {take_profit:.2f}, Entry: {entry_price:.2f}, SL: {stop_loss:.2f}")
                        self.breakout_signal = 0
                        return
                    ...
                    if risk_per_share > 0:
                        ...
                        if position_size > 0:
                            ...
                            print(f"🌙 MOON DEV: ENTERED SHORT - Size: {position_size}, Entry: {entry_price:.2f}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}")
                            self.breakout_signal = 0
