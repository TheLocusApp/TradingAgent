...
class VolatilityBreakout(Strategy):
    ...
    bb_period = 14  # Reduced for faster response
    bb_std = 1.8    # Tighter bands for better breakouts
    rsi_period = 12 # Faster RSI for momentum
    ...
    atr_multiplier_sl = 1.5  # Tighter stops with ATR
    ...
    def next(self):
        ...
        if not self.position:
            if is_squeeze and volume_confirmed:
                # 🌙 IMPROVED ENTRY CONDITIONS WITH TREND FILTER
                if current_close > bb_upper and rsi < 75 and rsi > 40 and uptrend:
                    self.breakout_signal = 1
                    print(f"🌙 MOON DEV: STRONG LONG BREAKOUT - Close: {current_close:.2f}, BB Upper: {bb_upper:.2f}, RSI: {rsi:.1f}, Volume: {current_volume:.0f}")
                    
                elif current_close < bb_lower and rsi > 25 and rsi < 60 and downtrend:
                    self.breakout_signal = -1
                    print(f"🌙 MOON DEV: STRONG SHORT BREAKOUT - Close: {current_close:.2f}, BB Lower: {bb_lower:.2f}, RSI: {rsi:.1f}, Volume: {current_volume:.0f}")
                ...
                if self.breakout_signal == 1:
                    ...
                    if risk_per_share > 0:
                        ...
                        max_position_size = int(self.equity * 0.15 / entry_price)  # Increased max size
